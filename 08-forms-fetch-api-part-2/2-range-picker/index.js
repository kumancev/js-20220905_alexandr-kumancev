export default class RangePicker {
  ...
  _highlightRange() {
    ...
    let fromRange = new Date(
      ...
      this.selectedDate.from.getDate()
    ).toISOString();
    let endRange;
  ...
  renderRangePicker() {
    ...
    let controlLeft = selector.querySelector(
      ".rangepicker__selector-control-left"
    );
    let controlRight = selector.querySelector(
  ...
  openCalendar(event) {
    ...
    if (
      ...
      (event.target.dataset.element === "input" ||
        event.target.closest("[data-element=input]"))
    ) {
      isOpen = false;
    }
